"""
SmartTrolley Server — Render / Colab (Flask + ONNX)
====================================================
ZERO LOAD ON PI: All inference runs HERE on the server.
WhatsApp Priority: 1. WATI, 2. Twilio

Pi → Server:
  POST /pi/frame/<id>        — raw JPEG frame (ONNX inference here)
  POST /pi/sensor_data       — weight + Arduino status + product name
...
def _postprocess(outputs, orig_h, orig_w):
    raw = np.squeeze(outputs[0], axis=0).T
    scores = raw[:, 4:]
